...
function clear() {
    var url = "x-local://source/gge/libs/event_outlook_lib.js";
    DropFormsCache(url);
...
function get_message_template() {
    var template;
    InPlaceEval(LoadFileData(FilePath(AppDirectoryPath(), "source/gge/libs/event_outlook_lib/message_template")));
    return template; 
...
function get_ical_template() {
    var template;
    InPlaceEval(LoadFileData(FilePath(AppDirectoryPath(), "source/gge/libs/event_outlook_lib/ical_template")));
    return template;
}

function get_reminder_template() {
    var template;
    InPlaceEval(LoadFileData(FilePath(AppDirectoryPath(), "source/gge/libs/event_outlook_lib/reminder_template")));
    return template;
}
...
function set_ical_attendees( _sIcal, _sHTML ) {
    if (_sHTML != undefined)
    return StrReplace( _sIcal, "WT_ATTENDEES", _sHTML );
}

function set_ical_place( _sIcal, _sHTML ) {
    if (_sHTML != undefined)
    return StrReplace( _sIcal, "WT_PLACE", _sHTML );
}
...
function add_reminder( _sIcal, _nDays, _sDesc, _sUID ) {
    var sReminderTemplate = get_reminder_template();
...
function create_event (_oConfig) {
    ...
    if (_oConfig.GetOptProperty("attendees", undefined) != undefined) {
        sAttendees = "";
        for (attendee in _oConfig.attendees) {
            sAttendees += 'ATTENDEE;ROLE=REQ-PARTICIPANT;PARTSTAT=NEEDS-ACTION;CN="' + attendee.fullname + '";RSVP=FALSE';
            sAttendees += ' :mailto:' + attendee.email + '\n';
        }
        sIcal = set_ical_attendees(sIcal, sAttendees);
    }
    
    if (_oConfig.GetOptProperty("place", undefined) != undefined)
    sIcal = set_ical_place(sIcal, _oConfig.place);

    prepare_ical_string_to_send(sIcal);
