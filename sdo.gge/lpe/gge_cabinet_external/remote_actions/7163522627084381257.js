        ...
        for (sField in aFieldCustomRequired) {
            ...
            if (StrContains(sField, 'date', true)) {
                ...
                if (docUser.TopElem.access.access_role == 'expert_gge') { //проверка заполнения обязательных кастомных полей для эксперта
                    _check = true;
                    if (docUser.TopElem.org_id == '') {
                        _check = false;
                    } //проверка выбора организации
                    if (docUser.TopElem.position_name == '') {
                        _check = false;
                    } //проверка указания должности
                    if (teUser.custom_elems.ObtainChildByKey('direction').value == '') {
                        _check = false;
                    } //проверка выбора направления
                    if (teUser.custom_elems.ObtainChildByKey('cur_status').value == '') {
                        _check = false;
                ...
                if (docUser.TopElem.access.access_role == 'corporate_customer_gge') { //проверка заполнения обязательных кастомных полей для корпоративного заказчика обучения
                    ...
                    if (teUser.custom_elems.ObtainChildByKey('educ_level').value == '') {
                        _check = false;
                    } //проверка выбора уровня образования
                    if (teUser.custom_elems.ObtainChildByKey('educ_scan').value == '') {
                        _check = false;
                    } //проверка наличия скана об образовании
                    if (teUser.custom_elems.ObtainChildByKey('educ_decl').value == '') {
                        _check = false;
                    } //проверка наличия скана заявления о зачислении
                    if (teUser.custom_elems.ObtainChildByKey('educ_pass').value == '') {
                        _check = false;
                    } //проверка наличия скана паспорта
                    if (teUser.custom_elems.ObtainChildByKey('snils').value == '') {
                        _check = false;
                    } //проверка наличия номера СНИЛС
                    if (teUser.custom_elems.ObtainChildByKey('snils_scan').value == '') {
                        _check = false;
                    } //проверка наличия скана СНИЛС
                    if (teUser.custom_elems.ObtainChildByKey('fio_dat').value == '') {
                        _check = false;
                    } //проверка указания ФИО в дательном падеже
                    if (teUser.custom_elems.ObtainChildByKey('user_ind').value == '') {
                        _check = false;
                    } //проверка указания индекса
                    if (teUser.custom_elems.ObtainChildByKey('city').value == '') {
                        _check = false;
                    } //проверка указания города
                    if (teUser.custom_elems.ObtainChildByKey('building').value == '') {
                        _check = false;
                    } //проверка указания улицы, номера дома, квартиры
                    if (_check) //если проверки пройдены
                    {
                        _tasks = XQuery('for $elem in tasks where $elem/executor_id=' + teUser.id + ' return $elem'); //выбор задач пользователя
                        for (_userTasks in _tasks) { //обход задач пользователя
